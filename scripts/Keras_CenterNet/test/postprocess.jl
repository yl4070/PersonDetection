...
function getprediction(model, imgx)
    ...
    yhat = model(imgx |> gpu) |> cpu
    # yhat[4]

    yhat1 = maximum(yhat[1], dims = 3)
    # maximum(yhat1)
    idx = findall(el -> el >= 0.3, yhat1[:,:,1,1])
    ...
    # end
    confs = map(idx) do id
        yhat1[id, 1, 1] # heatmap
    end

    classes = map(idx) do id
        Flux.onecold(yhat[1][id, :, 1], 1:20) # heatmap
    end
    ...
    detections = []
    for (id, szv, szh, offv, offh, conf, class) in zip(idx, sizevs, sizehs, offvs, offhs, confs, classes)
        ...
        push!(detections, (x=x, y=y, w=szh, h=szv, conf=conf, class = string(class)))
    end
