...
model = Chain(
    backbone,
    Split(
        Chain(ResBlock(67, 3), convxlayer(3, 2; ks = 1)), # offset
        Chain(ResBlock(67, 3), convxlayer(3, 2; ks = 1)), # size
        Chain(ResBlock(67, 3), convxlayer(3, 1; ks = 1)), # heatmap
        # Chain(ResBlock(67, 3), convxlayer(3, 2; ks = 1)), # segmentation
        Chain(ResBlock(67, 3), convdirlayer(3, 2; ks = 1))  # evidential
    )
)
